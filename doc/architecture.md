...
PayPay（QR決済）は個人向けの API を出していない。マネーフォワードや Zaim でも
自動連携できず、どこもアプリが出す取引履歴 CSV を読む形で対応している。
カード会社の「ご利用明細 CSV」も形は同じなので、PayPay 専用にはしない。

```
settings/DataSettings.jsx → components/CsvImport.jsx
  → utils/parseStatementCsv.js  CSV → 支出の行（列は見出しから自動判定）
  → utils/inbox.js の ingestDrafts  受信箱へ（前に取り込んだぶんは足さない）
  → components/InboxCard.jsx    クレカタブの「未確定の支出」
```

読んだものはそのまま家計に入れず、通知から来たものと同じ受信箱に置く。
カテゴリ付けも取り消しも同じ画面でできる。

- 列の名前は出どころで違う（PayPay は英語、カード会社は日本語）。見出しから
  当てて、外れたら画面で選び直せる。決め打ちにすると出どころが増えるたびに
  分岐が増える
- 文字コードは UTF-8 で読めなければ Shift_JIS（カード会社はまだ多い）
- 支出の符号は中身から決める。マイナスが 1 件でもあれば「マイナス＝支出」で、
  チャージ・入金は落とす。画面のチェックで反転できる
...
残る手が、アプリの利用履歴画面をそのまま読むこと（下記）。

## モバイルSuica の利用履歴を読み取る

```
settings/DataSettings.jsx → components/SuicaImport.jsx
  → utils/ocr.js                 画像 → 行（OcrPlugin / ML Kit 日本語）
  → utils/parseSuicaHistory.js   行 → 支出（残額の差から額を出す）
  → utils/inbox.js の ingestDrafts  受信箱へ
```

読み取りは端末内で終わる。モデルは Google Play 開発者サービスが持つので、
APK は太らず、画像もデータも外へ出ない（アプリ版だけの機能）。

画面に出るのは **月日・種別・利用場所・残額** の 4 つで、**使った額は出ない**。
だから残額の差から出す。
...
- 残額が動かない行（入場）は取り込まない
- 残額が増える行（チャージ）は取り込まない。家計から出たのはチャージ元の側
- いちばん古い行は「ひとつ前」が無いので額を出せない
