...
## 進め方（手戻りとトークンを減らす）
- **指示を受けたら、着手前にまず「何をやるか」の 1 行だけを先に出す。** 本文・見積・作業はその後に回す。
  1 行目に書いていても、長い本文と同じ応答にまとめると書き終えるまで画面に出ない（13:18 に実際に起きた）
...
## 見積（全作業で適用。詳細は `doc/estimation.md`）
- 依頼を受けたら最初に `TZ=Asia/Tokyo date` を呼び、原文を見積の 1 行目に貼る
- 下限 = Σ 工程（数量 × 実測）。数量は指示文と対象の実数、実測は `doc/estimation.md` §6 の一致する行のみ。
  指示に無い工程を足さない。動詞が要求する工程（分析・調査・修正）は 0 にしない
- 上限 = 下限 × 係数（同じ成果物の直近 10 行の実績÷下限の中央値、1.0〜2.0。3 行未満は 1.5）。守るのは上限
- 工程表を必ず出す。上限 15 分以上は半分の時刻に 1 行で中間報告
- 予備の上乗せ禁止。指摘されて数字だけ動かさない（変える工程・根拠を書く）
- 完了時に `date` を取り、§5 ログに 1 行足す
